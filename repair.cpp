...
void getSize(const string& file, int p, int min_valid_disk, int file_per_disk, int num_erasures, int* disks, 
    ...
    sprintf(file_name, "disk_%d/%s.%d", min_valid_disk, file.c_str(), file_per_disk-1);
    stat(file_name, &st);
    ...
    if(min_valid_disk != p-1){
        ...
        else d = disks[1] == p+1 ? (disks[0] == p ? p-1 : p) : p+1;
...
void repairFile(const string& filename, int failed_num, int* failed_disks, int p, int file_id, size_t file_size, size_t remain_size){
    ...
    if(failed_num == 1){
        ...
        if(failed_disks[1] == p+1){
            ...
            readDataColumn(filename.c_str(), p, file_id, file_size, R); // 初始化 R[0:p-1] = row_parity
            ...
            repairToDisk(filename.c_str(), missed_1, file_size, failed_disks[0], file_id, p);
            if(failed_disks[1] == p-1){
                char* remain_buffer = new char[remain_size]; // <= p * (p-1)
                readRemain(filename.c_str(), p, file_id, p, remain_size, remain_buffer);
                repairToDisk(filename.c_str(), missed_2, file_size, failed_disks[1], file_id, p, remain_buffer, remain_size);
                delete[] remain_buffer;
            }else repairToDisk(filename.c_str(), missed_2, file_size, failed_disks[1], file_id, p);
...
void repair(int num_erasures, int* disks){
    ...
    while((entry = readdir(dir_ptr))){
        if(entry->d_name[0] == '.')continue; // skip . and .. ; require: 测试文件命名不以 . 开头
    ...
    for(string& file:files){
        ...
        do{
            file_per_disk++;
            sprintf(file_name, "disk_%d/%s.%d", min_valid_disk, file.c_str(), file_per_disk);
        ...
        printf("file_size = %ld, remain size = %ld, last file size = %ld, last remain size = %ld\n",
               file_size, remain_size, last_file_size, last_remain_size);

        for(int i = 0;i < file_per_disk;i++){
            // TODO: last i should last_file_size and last_remain_size
            if(i != file_per_disk-1) repairFile(file, num_erasures, disks, p, i, file_size, remain_size);
            else repairFile(file, num_erasures, disks, p, i, last_file_size, last_remain_size);
        }
    }
}
