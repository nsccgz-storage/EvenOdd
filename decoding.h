...
size_t writeDataColumn(char* src, int id, size_t size, FILE* stream, int* reset_ptr){
    if(*reset_ptr){
...
void read1(char* filename, char* save_as){
    ...
    if(failed_num == 0){
        ...
        if(failed[1] == p+1){
            ...
            char* buffer = (char*)malloc(file_size);
            char* missed_column = (char*)malloc(file_size + block_size); // length = p
            ...
            readDataColumn(filename, p+1, file_size, D); // 初始化 D[0:p-1] = diagonal_parity
            ...
            do{
                ...
                // 第一次进入循环时其实执行 D1 ^ R[p-1] = missed_2[m]，因此之前需要初始化 R[p-1] = 0
